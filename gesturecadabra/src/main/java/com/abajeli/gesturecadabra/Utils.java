...
public class Utils {
    ...
    private static final long[] DEFAULT_VIBRATION_PATTERN =  {0, 300, 0, 100}; // in millis
    ...
    /**
     * Causes device to vibrate following the given pattern. If pattern is empty it
     * will use the <code>DEFAULT_VIBRATION_PATTERN</code>.
     * //   don't repeat if inPatternToRepeat is -1
     */
    public final static void vibratePattern(Context context, long[] pattern, int inPatternToRepeat) {
        if (pattern.length == 0) {
            pattern = DEFAULT_VIBRATION_PATTERN;
        }
        Vibrator v = (Vibrator) context.getSystemService(Context.VIBRATOR_SERVICE);
        v.vibrate(pattern, inPatternToRepeat);
    }
